...
def parser():
    ...
    parser = argparse.ArgumentParser(
        prog='data_prep.py', description='generates GW data for application of deep learning networks.')

    # arguments for reading in a data file
    parser.add_argument('-N', '--Nsamp', type=int,
                        default=7000, help='the number of samples')
    ...
    parser.add_argument('-f', '--fsample', type=int,
                        default=8192, help='the sampling frequency (Hz)')
    parser.add_argument('-T', '--Tobs', type=int, default=1,
                        help='the observation duration (sec)')
    parser.add_argument('-s', '--snr', type=float,
                        default=None, help='the signal integrated SNR')
    parser.add_argument('-I', '--detectors', type=str, nargs='+',
                        default=['H1', 'L1'], help='the detectors to use')
    parser.add_argument('-b', '--basename', type=str,
                        default='test', help='output file path and basename')
    parser.add_argument('-m', '--mdist', type=str, default='astro',
                        help='mass distribution for training (astro,gh,metric)')
    parser.add_argument('-z', '--seed', type=int,
                        default=1, help='the random seed')
...
def gen_psd(fs, T_obs, op='AdvDesign', det='H1'):
    ...
    psd = lal.CreateREAL8FrequencySeries(
        None, lal.LIGOTimeGPS(0), 0.0, df, lal.HertzUnit, N // 2 + 1)
...
def gen_masses(m_min=5.0, M_max=100.0, mdist='astro', verbose=True):
    ...
    if mdist == 'astro':
        ...
        while not flag:
            m12 = np.exp(np.log(new_m_min) + np.random.uniform(0,
                         1, 2)*(log_m_max-np.log(new_m_min)))
            flag = bool(
        ...
        while not flag:
            ...
            m12[0] = m12[1]*q
            flag = bool((np.all(m12 < 75.0)) and (
                np.all(m12 > 5.0)) and (m12[0] >= m12[1]))
        eta = m12[0]*m12[1]/(m12[0]+m12[1])**2
        mc = np.sum(m12)*eta**(3.0/5.0)
        return m12, mc, eta
    elif mdist == 'metric':
...
def make_bbh(hp, hc, fs, ra, dec, psi, det, verbose):
    ...
    resp = AntennaResponse(
        det, ra, dec, psi, scalar=True, vector=True, times=0.0)
    Fp = resp.plus
    Fc = resp.cross
...
def sim_data(fs, T_obs, snr=1.0, dets=['H1'], Nnoise=25, size=1000, mdist='astro', beta=[0.75, 0.95], verbose=True):
    ...
    while cnt < size:
        ...
        ts_new, _, _ = gen_bbh(
            fs, T_obs, psds, snr=snr, dets=dets, beta=beta, par=par_new, verbose=verbose)
...
def main():
    ...
    for i in xrange(nblock):
        ...
        ts, par = sim_data(args.fsample, safeTobs, args.snr, args.detectors,
                           args.Nnoise, size=args.Nblock, mdist=args.mdist, beta=[0.75, 0.95])
